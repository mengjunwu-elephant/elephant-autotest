...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMyHand.TEST_DATA_FILE, "get_gripper_joint_mini_pressure")
...
class TestGetGripperJointMiniPressure(unittest.TestCase):
    ...
    @data(*cases)
    def test_get_gripper_joint_mini_pressure(self, case):
        ...
        # 请求发送
        response = self.device.m.get_gripper_joint_mini_pressure(case["joint"])
    ...
    def test_out_limit(self,case):
        ...
        # 请求发送
        try:
            with self.assertRaises(ValueError, msg="用例{}未触发value错误，最小启动力值为{}".format(case['title'], case['joint'])):
                self.device.m.get_gripper_joint_mini_pressure(int(case['joint']))
