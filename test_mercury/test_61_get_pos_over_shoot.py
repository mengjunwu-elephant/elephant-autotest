...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.TEST_DATA_FILE, "get_pos_over_shoot")


@ddt
class TestGetPosOverShoot(unittest.TestCase):
    # 实例化日志模块
    logger = logger
    ...
    def test_get_pos_over_shoot(self, case):
        ...
        # 左臂请求发送
        l_response = self.device.ml.get_pos_over_shoot()

        # 右臂请求发送
        r_response = self.device.mr.get_pos_over_shoot()
        try:
            # 请求结果类型断言
