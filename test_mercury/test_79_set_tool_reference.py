...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.TEST_DATA_FILE, "set_tool_reference")


@ddt
class TestSetToolReference(unittest.TestCase):
    # 实例化日志模块
    logger = logger
    ...
    def test_set_tool_reference(self, case):
        ...
        # 左臂请求发送
        l_response = self.device.ml.set_tool_reference(case['parameter'])
        self.device.ml.set_end_type(1)
        # 右臂请求发送
        r_response = self.device.mr.set_tool_reference(case['parameter'])
        self.device.mr.set_end_type(1)
        try:
            # 请求结果类型断言
