...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestProGripper.TEST_DATA_FILE, "set_open_angle")


@ddt
class TestSetOpenAngle(unittest.TestCase):
    # 实例化日志模块
    logger = logger
    ...
    def test_set_open_angle(self, case):
        ...
        # 请求发送
        response = self.device.m.set_gripper_io_open_value(case["parameter"])
    ...
    def test_out_limit(self,case):
        ...
        try:
            with self.assertRaises(ValueError, msg="用例{}未触发value错误，value值为{}".format(case['title'], case['parameter'])):
                self.device.m.set_gripper_io_open_value(case["parameter"])
