...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestProGripper.TEST_DATA_FILE, "set_protection_current")


@ddt
class TestSetProtectionCurrent(unittest.TestCase):
    # 实例化日志模块
    logger = logger
    ...
    def test_set_protection_current(self, case):
        ...
        # 请求发送
        set_res = self.device.m.set_gripper_protection_current(case["parameter"])
        get_res = self.device.m.get_gripper_protection_current()
        try:
            # 请求结果类型断言
            if type(set_res) == int:
    ...
    def test_out_limit(self, case):
        ...
        try:
            with self.assertRaises(ValueError,
                ...
                self.device.m.set_gripper_protection_current(case["parameter"])
