...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestProGripper.TEST_DATA_FILE, "set_gripper_i")


@ddt
class TestSetGripperI(unittest.TestCase):
    # 实例化日志模块
    logger = logger
    ...
    def test_set_gripper_i(self, case):
        ...
        # 请求发送
        set_res = self.device.m.set_gripper_I(case["parameter"])
        get_res = self.device.m.get_gripper_I()
