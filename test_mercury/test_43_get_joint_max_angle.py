...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.TEST_DATA_FILE, "get_joint_max_angle")


@ddt
class TestGetJointMaxAngle(unittest.TestCase):
    ...
    def test_get_joint_max_angle(self, case):
        ...
        # 右臂请求发送
        r_response = self.device.mr.get_joint_max_angle(case["id"])
        self.device.mr.send_angle(case["id"], case["r_expect_data"], self.device.speed)
        sleep(3)
        ...
        try:
            ...
            logger.debug("左臂{}关节软件限位值为{}，当前角度值为{}".format(case['id'], case["l_expect_data"],
                                                                               self.device.ml.get_angle(case["id"])))
            logger.debug("右臂{}关节软件限位值为{}，当前角度值为{}".format(case['id'], case["r_expect_data"],
    ...
    def test_out_limit(self, case):
        ...
        try:
            with self.assertRaises(MercuryDataException, msg="用例{}未触发value错误，id值为{}".format(case['title'], case['id'])):
                self.device.ml.get_joint_max_angle(case["id"])
                self.device.mr.get_joint_max_angle(case["id"])
