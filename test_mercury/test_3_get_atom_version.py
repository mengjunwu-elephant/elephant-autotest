...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.TEST_DATA_FILE, "get_atom_version")


@ddt
class TestGetAtomVersion(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        """
        水星系列初始化先左臂上电，后右臂上电
    ...
    def test_get_atom_version(self, case):
        ...
        self.device.reset()
        # 左臂请求发送
        l_response = self.device.ml.get_atom_version()
        # 右臂请求发送
        r_response = self.device.mr.get_atom_version()
        try:
            # 请求结果类型断言
