...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.TEST_DATA_FILE, "set_joint_min_angle")


@ddt
class TestSetJointMinAngle(unittest.TestCase):
    # 实例化日志模块
    logger = logger
    ...
    def test_set_joint_min_angle(self, case):
        ...
        # 右臂请求发送
        r_response = self.device.mr.set_joint_min_angle(case["id"])
        self.device.mr.send_angle(case["id"], case["parameter"])
        sleep(3)
        ...
        try:
            ...
            self.logger.debug("左臂{}关节设置的软件限位值为{}，当前角度值为{}".format(case['id'], case["parameter"],
                                                                                     self.device.ml.get_angle(
                                                                                         case["id"])))
            self.logger.debug("右臂{}关节设置的软件限位值为{}，当前角度值为{}".format(case['id'], case["parameter"],
