...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.TEST_DATA_FILE, "stop")
...
class TestStop(unittest.TestCase):
    ...
    def test_stop(self, case):
        ...
        # 左臂请求发送
        l_response = self.device.ml.stop(case['parameter'])

        # 右臂请求发送
        r_response = self.device.mr.stop(case['parameter'])
        try:
            # 请求结果类型断言
    ...
    def test_out_limit(self, case):
        ...
        try:
            with self.assertRaises(MercuryDataException,
                ...
                self.device.ml.stop(case['parameter'])
                self.device.mr.stop(case['parameter'])
