...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.PRO_GRIPPER_TEST_DATA_FILE, "set_pro_gripper_calibration")


@ddt
class TestGetProGripperCalibration(unittest.TestCase):
    ...
    @data(*cases)
    def test_set_pro_gripper_calibration(self, case):
        ...
        # 请求发送
        response = self.device.ml.set_pro_gripper_calibration()
        try:
            # 请求结果类型断言
