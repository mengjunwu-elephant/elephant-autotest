...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.TEST_DATA_FILE, "get_control_mode")


@ddt
class TestGetControlMode(unittest.TestCase):
    # 实例化日志模块
    logger = logger
    ...
    def test_get_control_mode1(self, case):
        ...
        # 左臂请求发送
        l_response = self.device.ml.get_control_mode()

        # 右臂请求发送
        r_response = self.device.mr.get_control_mode()
        try:
            # 请求结果类型断言
    ...
    def test_get_control_mode2(self, case):
        self.logger.info('》》》》》用例【{}】开始测试《《《《《'.format(case['title']))
        # 调试信息
        self.logger.debug('test_api:{}'.format(case['api']))
        self.logger.debug('test_parameter:{}'.format(case['parameter']))
        # 切换为力矩模式
        self.device.ml.set_control_mode(1)
        self.device.mr.set_control_mode(1)
        # 左臂请求发送
        l_response = self.device.ml.get_control_mode()
