...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.TEST_DATA_FILE, "get_max_speed")
...
class TestGetMaxSpeed(unittest.TestCase):
    ...
    def test_get_max_speed(self, case):
        ...
        # 左臂请求发送
        l_response = self.device.ml.get_max_speed(case["parameter"])

        # 右臂请求发送
        r_response = self.device.mr.get_max_speed(case["parameter"])
        try:
            # 请求结果类型断言
    ...
    def test_out_limit(self, case):
        ...
        try:
            with self.assertRaises(MercuryDataException,
                ...
                self.device.ml.get_max_speed(case['parameter'])
                self.device.mr.get_max_speed(case['parameter'])
