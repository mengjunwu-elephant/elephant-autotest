...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.TEST_DATA_FILE, "solve_inv_kinematics")


@ddt
class TestSolveInvKinematics(unittest.TestCase):
    ...
    def test_solve_inv_kinematics(self, case):
        ...
        # 左臂请求发送
        l_response = self.device.ml.solve_inv_kinematics(case['parameter'],case['parameter_1'])

        # 右臂请求发送
        r_response = self.device.mr.solve_inv_kinematics(case['parameter'],case['parameter_1'])
        try:
            # 请求结果类型断言
    ...
    def test_out_limit(self, case):
        ...
        try:
            with self.assertRaises(MercuryDataException,
                                   msg="用例{}未触发value错误，参数为{}{}".format(case['title'], case['parameter'],case['parameter_1'])):
                self.device.ml.solve_inv_kinematics(case['parameter'],case['parameter_1'])
                self.device.mr.solve_inv_kinematics(case['parameter'],case['parameter_1'])
