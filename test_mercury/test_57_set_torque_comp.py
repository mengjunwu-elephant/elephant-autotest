...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.TEST_DATA_FILE, "set_torque_comp")


@ddt
class TestSetTorqueComp(unittest.TestCase):
    ...
    def tearDownClass(cls):
        ...
        :return:
        """
        cls.device.set_default_torque_comp()
    ...
    def test_set_torque_comp(self, case):
        ...
        # 左臂请求发送
        l_response = self.device.ml.set_torque_comp(case["joint"],case['parameter'])

        # 右臂请求发送
        r_response = self.device.mr.set_torque_comp(case["joint"],case['parameter'])
        try:
            # 请求结果类型断言
    ...
    def test_out_limit(self, case):
        ...
        try:
            with self.assertRaises(MercuryDataException,
                ...
                self.device.ml.set_torque_comp(case["joint"],case['parameter'])
                self.device.mr.set_torque_comp(case["joint"],case['parameter'])
    ...
    def test_save_or_not(self, case):
        ...
        # 读取默认值
        l_get_res = self.device.ml.get_torque_comp()
        r_get_res = self.device.mr.get_torque_comp()
        try:
            # 请求结果类型断言
            if type(l_response) == int:
                logger.debug('左臂请求类型断言成功')
