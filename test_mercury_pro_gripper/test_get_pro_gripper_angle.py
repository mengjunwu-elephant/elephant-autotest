...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.PRO_GRIPPER_TEST_DATA_FILE, "get_pro_gripper_angle")


@ddt
class TestGetProGripperAngle(unittest.TestCase):
    ...
    @data(*cases)
    def test_get_pro_gripper_angle(self, case):
        ...
        # 请求发送
        response = self.device.ml.get_pro_gripper_angle()
        try:
            # 请求结果类型断言
