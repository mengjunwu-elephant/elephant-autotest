...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.TEST_DATA_FILE, "write_move_c")


@ddt
class TestSetSolutionAngles(unittest.TestCase):
    ...
    def test_write_move_c(self, case):
        logger.info('》》》》》用例【{}】开始测试《《《《《'.format(case['title']))
        # 调试信息
        logger.debug('test_api:{}'.format(case['api']))
        logger.debug('test_transpoint:{}'.format(case['transpoint']))
        logger.debug('test_endpoint:{}'.format(case['endpoint']))
        # 左臂请求发送
        r_response = self.device.mr.write_move_c(eval(case["transpoint"]),eval(case["endpoint"]), case["speed"])
        l_response = self.device.ml.write_move_c(eval(case["transpoint"]),eval(case["endpoint"]), case["speed"])
        try:
            # 请求结果类型断言
            ...
            if type(r_response) == int:
                ...
                logger.debug('右臂请求类型断言失败，实际类型为{}'.format(type(l_response)))
            # 请求结果断言
    ...
    def test_out_limit(self, case):
        ...
        try:
            with self.assertRaises(MercuryDataException,
                                   msg="用例{}未触发value错误，角度，速度为{}{}{}".format(case['title'], case['transpoint'],case['endpoint'],
                                                                                      case['speed'])):
                # 左臂请求发送
                self.device.ml.write_move_c(eval(case["transpoint"]),eval(case["endpoint"]), case["speed"])
                # 右臂请求发送
                self.device.mr.write_move_c(eval(case["transpoint"]),eval(case["endpoint"]), case["speed"])
