...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.MY_HAND_TEST_DATA_FILE, "get_hand_gripper_cww")


@ddt
class TestGetHandGripperCounterClockwise(unittest.TestCase):
    ...
    @data(*cases)
    def test_get_hand_gripper_counterclockwise(self, case):
        logger.info('》》》》》用例【{}】开始测试《《《《《'.format(case['title']))
        # 调试信息
        logger.debug('test_api:{}'.format(case['api']))
        logger.debug('test_joint:{}'.format(case['joint']))
        # 请求发送
        response = self.device.ml.get_hand_gripper_counterclockwise(case["joint"])
        try:
            # 请求结果类型断言
    ...
    def test_out_limit(self, case):
        ...
        try:
            with self.assertRaises(MercuryDataException,
                                   msg="用例{}未触发value错误，Cww值为{}".format(case['title'], case['joint'])):
                self.device.ml.get_hand_gripper_counterclockwise(case['joint'])
