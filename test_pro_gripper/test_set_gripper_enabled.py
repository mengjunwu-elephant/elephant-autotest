...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestProGripper.TEST_DATA_FILE, "set_gripper_enabled")


@ddt
class TestSetGripperEnabled(unittest.TestCase):
    # 实例化日志模块
    logger = logger
    ...
    def test_set_gripper_enabled(self, case):
        ...
        # 请求发送
        response = self.device.m.set_gripper_enable(case["parameter"])
        try:
            # 请求结果类型断言
    ...
    def test_out_limit(self, case):
        ...
        try:
            with self.assertRaises(ValueError,
                ...
                self.device.m.set_gripper_enable(case["parameter"])
