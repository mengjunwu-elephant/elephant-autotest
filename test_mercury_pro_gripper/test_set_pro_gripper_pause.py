...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.PRO_GRIPPER_TEST_DATA_FILE, "set_pro_gripper_pause")


@ddt
class TestSetProGripperPause(unittest.TestCase):
    ...
    def tearDownClass(cls):
        cls.device.ml.set_pro_gripper_resume()
        cls.device.ml.set_pro_gripper_angle(0)
        cls.device.ml.close()
        logger.info("环境清理完成，接口测试结束")

    @data(*cases)
    def test_set_pro_gripper_pause(self, case):
        ...
        # 请求发送
        self.device.ml.set_pro_gripper_angle(100)
        sleep(0.2)
        response = self.device.ml.set_gripper_set_pro_gripper_pause()
        try:
            # 请求结果类型断言
