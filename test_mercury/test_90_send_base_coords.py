...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.TEST_DATA_FILE, "send_base_coords")


@ddt
class TestSendBaseCoords(unittest.TestCase):
    ...
    @data(*[case for case in cases if case.get("test_type") == "left"])
    def test_send_base_coords_left(self, case):
        ...
        # 左臂请求发送
        l_response = self.device.ml.send_base_coords(eval(case["parameter"]),case["speed"])
        try:
            # 请求结果类型断言
    ...
    @data(*[case for case in cases if case.get("test_type") == "right"])
    def test_send_base_coords_right(self, case):
        logger.info('》》》》》用例【{}】开始测试《《《《《'.format(case['title']))
        # 调试信息
        logger.debug('test_api:{}'.format(case['api']))
        logger.debug('test_parameter_1:{}'.format(case['parameter']))
        logger.debug('test_parameter_2:{}'.format(case['speed']))
        # 右臂请求发送
        r_response = self.device.mr.send_base_coords(eval(case["parameter"]), case["speed"])
        try:
            # 请求结果类型断言
    ...
    @data(*[case for case in cases if case.get("test_type") == "exception_left"])  # 筛选无效等价类用例
    def test_out_limit_left(self, case):
        ...
        try:
            with self.assertRaises(MercuryDataException,
                ...
                # 左臂请求发送
                self.device.ml.send_base_coords(eval(case["parameter"]), case["speed"])
    ...
    @data(*[case for case in cases if case.get("test_type") == "exception_right"])  # 筛选无效等价类用例
        ...
        try:
            with self.assertRaises(MercuryDataException,
                                   msg="用例{}未触发value错误，坐标，速度为{}{}".format(case['title'], case['parameter'],
                                                                                      case['speed'])):
                # 右臂请求发送
                self.device.mr.send_base_coords(eval(case["parameter"]), case["speed"])
        except AssertionError:
            logger.error("断言失败：用例{}未触发异常".format(case['title']))
            raise  # 重新抛出异常，让测试框架捕获
        except Exception as e:
