...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.TEST_DATA_FILE, "get_torque_comp")


@ddt
class TestGetTorqueComp(unittest.TestCase):
    # 实例化日志模块
    logger = logger
    ...
    def test_get_torque_comp(self, case):
        ...
        # 左臂请求发送
        l_response = self.device.ml.get_torque_comp()

        # 右臂请求发送
        r_response = self.device.mr.get_torque_comp()
        try:
            # 请求结果类型断言
    ...
    def test_out_limit(self, case):
        ...
        try:
            with self.assertRaises(MercuryDataException,
                ...
                self.device.ml.get_torque_comp()
                self.device.mr.get_torque_comp()
