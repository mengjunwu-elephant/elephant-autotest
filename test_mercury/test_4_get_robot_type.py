...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.TEST_DATA_FILE, "get_robot_type")


@ddt
class TestGetRobotType(unittest.TestCase):
    ...
    def test_get_robot_type(self, case):
        logger.info('》》》》》用例【{}】开始测试《《《《《'.format(case['title']))
        # 调试信息
        logger.debug('test_api:{}'.format(case['api']))
        logger.debug('test_parameters:{}'.format(case['parameters']))
        # 左臂请求发送
        l_response = self.device.ml.get_robot_type()
        # 右臂请求发送
        r_response = self.device.mr.get_robot_type()
        try:
            # 请求结果类型断言
