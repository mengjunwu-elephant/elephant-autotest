...
class TestSetTorqueComp(unittest.TestCase):
    ...
    def test_set_torque_comp(self, case):
        ...
        # 左臂请求发送
        l_response = self.device.ml.set_torque_comp(case['parameter'])

        # 右臂请求发送
        r_response = self.device.mr.set_torque_comp(case['parameter'])
        try:
            # 请求结果类型断言
    ...
    def test_out_limit(self, case):
        ...
        try:
            with self.assertRaises(MercuryDataException,
                ...
                self.device.ml.set_torque_comp(case['parameter'])
                self.device.mr.set_torque_comp(case['parameter'])
