...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.TEST_DATA_FILE, "set_servo_calibration")


@ddt
class TestSetServoCalibration(unittest.TestCase):
    # 实例化日志模块
    logger = logger
    ...
    @data(*cases)
    def test_set_servo_calibration(self, case):
        ...
        # 左臂请求发送
        l_response = self.device.ml.set_servo_calibration(case["id"])

        # 右臂请求发送
        r_response = self.device.mr.set_servo_calibration(case["id"])
    ...
    def test_out_limit(self, case):
        ...
        try:
            with self.assertRaises(ValueError,
                                   msg="用例{}未触发value错误，关节值为{}".format(case['title'], case['id'])):
                self.device.ml.set_servo_calibration(case["id"])
                self.device.mr.set_servo_calibration(case["id"])
