...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.MY_HAND_TEST_DATA_FILE, "set_hand_gripper_i")


@ddt
class TestSetHandGripperI(unittest.TestCase):
    ...
    def test_set_hand_gripper_i(self, case):
        ...
        # 请求发送
        set_res = self.device.ml.set_hand_gripper_i(case["joint"], case["parameter"])
        get_res = self.device.ml.get_hand_gripper_i(case["joint"])
        try:
            # 请求结果类型断言
            if type(set_res) == int:
    ...
    def test_out_limit(self, case):
        ...
        try:
            with self.assertRaises(MercuryDataException,
                                   msg="用例{}未触发value错误，I值为{}".format(case['title'], case['parameter'])):
                self.device.ml.set_hand_gripper_i(case["joint"], case["parameter"])
