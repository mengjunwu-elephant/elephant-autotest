...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestProGripper.TEST_DATA_FILE, "set_gripper_id")


@ddt
class TestSetGripperID(unittest.TestCase):
    # 实例化日志模块
    logger = logger
    ...
    def tearDownClass(cls):
        cls.device.m.set_gripper_Id(14)  # 恢复默认ID
        cls.device.m.close()
        cls.logger.info("环境清理完成，接口测试结束")

    def tearDown(self):
        self.device.m.set_gripper_Id(14)


    @data(*cases)
    def test_set_gripper_id(self, case):
        self.logger.info('》》》》》用例【{}】开始测试《《《《《'.format(case['title']))
        # 调试信息
        self.logger.debug('test_api:{}'.format(case['api']))
        self.logger.debug('test_parameters:{}'.format(case['id']))
        # 请求发送
        set_res = self.device.m.set_gripper_Id(int(case["id"]))
        get_res = self.device.m.get_gripper_Id()
        # 请求结果类型断言
        if type(set_res) == int:
        ...
        try:
            ...
            self.assertEqual(get_res, case["id"])
        except AssertionError as e:
            self.logger.exception('请求结果断言失败')
            self.logger.debug('期望数据：{},{}'.format(case['expect_data'],case['id']))
            self.logger.debug('实际结果：{},{}'.format(set_res,get_res))
            raise e
        else:
    ...
    def test_out_limit(self,case):
        ...
        try:
            with self.assertRaises(ValueError, msg="用例{}未触发value错误，value值为{}".format(case['title'], case['parameter'])):
                self.device.m.set_gripper_I(case["parameter"])
