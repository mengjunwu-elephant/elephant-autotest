...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.TEST_DATA_FILE, "get_servo_encoder")


@ddt
class TestGetServoEncoder(unittest.TestCase):
    ...
    @data(*cases)
    def test_get_servo_encoder(self, case):
        logger.info('》》》》》用例【{}】开始测试《《《《《'.format(case['title']))
        # 调试信息
        logger.debug('test_api:{}'.format(case['api']))
        logger.debug('test_api:{}'.format(case['joint']))
        # 左臂请求发送
        l_response = self.device.ml.get_servo_encoder(case['joint'])

        # 右臂请求发送
        r_response = self.device.mr.get_servo_encoder(case['joint'])
