...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.TEST_DATA_FILE, "jog_coord")


@ddt
class TestJogCoord(unittest.TestCase):
    ...
    def test_jog_coord(self, case):
        ...
        # 左臂请求发送
        l_response = self.device.ml.jog_coord(case["axis"],case["parameter"],case["speed"])
        # 右臂请求发送
        r_response = self.device.mr.jog_coord(case["axis"],case["parameter"], case["speed"])
        try:
            # 请求结果类型断言
    ...
    def test_out_limit(self, case):
        ...
        try:
            with self.assertRaises(MercuryDataException,
                ...
                # 左臂请求发送
                self.device.ml.jog_coord(case["axis"],case["parameter"], case["speed"])
