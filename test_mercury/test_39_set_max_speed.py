...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.TEST_DATA_FILE, "set_max_speed")
...
class TestSetMaxSpeed(unittest.TestCase):
    ...
    def test_set_max_speed(self, case):
        ...
        # 左臂请求发送
        l_response = self.device.ml.set_max_speed(case['parameter'],case['mode'])

        # 右臂请求发送
        r_response = self.device.mr.set_max_speed(case['parameter'],case['mode'])
        try:
            # 请求结果类型断言
    ...
    def test_out_limit(self, case):
        ...
        try:
            with self.assertRaises(MercuryDataException,
                ...
                self.device.ml.set_max_speed(case['parameter'],case['mode'])
                self.device.mr.set_max_speed(case['parameter'],case['mode'])
