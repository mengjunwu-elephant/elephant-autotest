...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.TEST_DATA_FILE, "get_collision_mode")


@ddt
class TestGetCollisionMode(unittest.TestCase):
    ...
    def test_get_collision_mode(self, case):
        ...
        # 左臂请求发送
        l_response = self.device.ml.get_collision_mode()

        # 右臂请求发送
        r_response = self.device.mr.get_collision_mode()
        try:
            # 请求结果类型断言
