...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.TEST_DATA_FILE, "get_servo_status")


@ddt
class TestGetServoStatus(unittest.TestCase):
    ...
    def test_get_servo_status(self, case):
        ...
        # 左臂请求发送
        l_response = self.device.ml.get_servo_status()

        # 右臂请求发送
        r_response = self.device.mr.get_servo_status()
        try:
            # 请求结果类型断言
