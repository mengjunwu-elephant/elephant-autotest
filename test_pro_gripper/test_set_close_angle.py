...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestProGripper.TEST_DATA_FILE, "set_close_angle")


@ddt
class TestSetCloseAngle(unittest.TestCase):
    # 实例化日志模块
    logger = logger
    ...
    def test_set_close_angle(self, case):
        ...
        # 请求发送
        response = self.device.m.set_gripper_io_close_value(case["parameter"])
        try:
            # 请求结果类型断言
