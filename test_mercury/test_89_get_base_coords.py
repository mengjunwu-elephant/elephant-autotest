...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.TEST_DATA_FILE, "get_base_coords")


@ddt
class TestGetBaseCoords(unittest.TestCase):
    # 实例化日志模块r
    logger = logger
    ...
    @data(*cases)
    def test_get_base_coords(self, case):
        logger.info('》》》》》用例【{}】开始测试《《《《《'.format(case['title']))
        # 调试信息
        logger.debug('test_api:{}'.format(case['api']))
        # 左臂请求发送
        l_response = self.device.ml.get_base_coords()
        # 右臂请求发送
        r_response = self.device.mr.get_base_coords()
