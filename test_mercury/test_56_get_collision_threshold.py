...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.TEST_DATA_FILE, "get_collision_threshold")


@ddt
class TestGetCollisionThreshold(unittest.TestCase):
    # 实例化日志模块
    logger = logger
    ...
    def test_get_collision_threshold(self, case):
        ...
        # 左臂请求发送
        l_response = self.device.ml.get_collision_threshold()

        # 右臂请求发送
        r_response = self.device.mr.get_collision_threshold()
        try:
            # 请求结果类型断言
    ...
    def test_out_limit(self, case):
        ...
        try:
            with self.assertRaises(MercuryDataException,
                ...
                self.device.ml.get_collision_threshold()
                self.device.mr.get_collision_threshold()
