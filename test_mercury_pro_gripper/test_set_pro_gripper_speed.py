...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.TEST_DATA_FILE, "set_pro_gripper_speed")


@ddt
class TestSetProGripperSpeed(unittest.TestCase):
    ...
    def tearDownClass(cls):
        cls.device.ml.set_pro_gripper_speed(100)
        cls.device.ml.close()
        logger.info("环境清理完成，接口测试结束")

    @data(*[case for case in cases if case.get("test_type") == "normal"])  # 筛选有效等价类用例
    def test_set_pro_gripper_speed(self, case):
        ...
        # 请求发送
        response = self.device.ml.set_pro_gripper_speed(case["parameter"])
        try:
            # 请求结果类型断言
    ...
    def test_out_limit(self, case):
        ...
        try:
            with self.assertRaises(ValueError,
                ...
                self.device.ml.set_pro_gripper_speed(case["parameter"])
