...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestProGripper.TEST_DATA_FILE, "get_open_angle")


@ddt
class TestGetOpenAngle(unittest.TestCase):
    ...
    @data(*cases)
    def test_get_open_angle(self, case):
        ...
        # 请求发送
        response = self.device.m.get_gripper_io_open_value()
        try:
            # 请求结果类型断言
