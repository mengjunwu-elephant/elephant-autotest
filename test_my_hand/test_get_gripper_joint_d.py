...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMyHand.TEST_DATA_FILE, "get_gripper_joint_d")


@ddt
class TestGetGripperJointD(unittest.TestCase):
    # 实例化日志模块
    logger = logger
    ...
    @data(*cases)
    def test_get_gripper_joint_d(self, case):
        ...
        # 请求发送
        response = self.device.m.get_gripper_joint_D(case["joint"])
    ...
    def test_out_limit(self,case):
        ...
        # 请求发送
        try:
            with self.assertRaises(ValueError, msg="用例{}未触发value错误，D值为{}".format(case['title'], case['joint'])):
                self.device.m.get_gripper_joint_D(int(case['joint']))
