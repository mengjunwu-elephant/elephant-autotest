...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.PRO_GRIPPER_TEST_DATA_FILE, "set_pro_gripper_resume")


@ddt
class TestSetProGripperResume(unittest.TestCase):
    ...
    @data(*cases)
    def test_set_pro_gripper_resume(self, case):
        ...
        # 请求发送
        response = self.device.ml.set_pro_gripper_resume()
        try:
            # 请求结果类型断言
