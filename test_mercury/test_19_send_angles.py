...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.TEST_DATA_FILE, "send_angles")


@ddt
class TestSendAngles(unittest.TestCase):
    # 实例化日志模块
    logger = logger
    ...
    def test_send_angles(self, case):
    ...
    def test_out_limit(self, case):
        ...
        try:
            with self.assertRaises(MercuryDataException,
                ...
                # 左臂请求发送
                self.device.ml.send_angles(eval(case["parameter"]), case["speed"])
                # 右臂请求发送
                self.device.mr.send_angles(eval(case["parameter"]), case["speed"])
