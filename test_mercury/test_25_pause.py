...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.TEST_DATA_FILE, "pause")
...
class TestPause(unittest.TestCase):
    ...
    def test_pause(self, case):
        ...
        # 左臂请求发送
        l_response = self.device.ml.pause(case['parameter'])

        # 右臂请求发送
        r_response = self.device.mr.pause(case['parameter'])
        try:
            # 请求结果类型断言
    ...
    def test_out_limit(self, case):
        ...
        try:
            with self.assertRaises(MercuryDataException,
                ...
                self.device.ml.pause(case['parameter'])
                self.device.mr.pause(case['parameter'])
