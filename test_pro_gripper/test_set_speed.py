...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestProGripper.TEST_DATA_FILE, "set_speed")
...
class TestSetSpeed(unittest.TestCase):
    ...
    def test_set_speed(self, case):
        ...
        # 请求发送
        response = self.device.m.set_gripper_speed(case["parameter"])
        try:
            # 请求结果类型断言
    ...
    def test_out_limit(self, case):
        ...
        try:
            with self.assertRaises(ValueError,
                ...
                self.device.m.set_gripper_speed(case["parameter"])
