...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.TEST_DATA_FILE, "send_base_coord")


@ddt
class TestSendBaseCoord(unittest.TestCase):
    ...
    def test_send_base_coord_left(self, case):
        ...
        # 左臂请求发送
        l_response = self.device.ml.send_base_coord(case["axis"],case["parameter"],case["speed"])
        try:
            # 请求结果类型断言
    ...
    def test_send_base_coord_right(self, case):
        logger.info('》》》》》用例【{}】开始测试《《《《《'.format(case['title']))
        # 调试信息
        logger.debug('test_api:{}'.format(case['api']))
        logger.debug('test_parameter_1:{}'.format(case['parameter']))
        logger.debug('test_parameter_2:{}'.format(case['speed']))
        # 右臂请求发送
        r_response = self.device.mr.send_base_coord(case["axis"],case["parameter"], case["speed"])
        try:
            # 请求结果类型断言
    ...
    def test_out_limit(self, case):
        ...
        try:
            with self.assertRaises(MercuryDataException,
                ...
                # 左臂请求发送
                self.device.ml.send_base_coord(case["axis"],case["parameter"], case["speed"])
