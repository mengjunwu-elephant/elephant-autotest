...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.MY_HAND_TEST_DATA_FILE, "set_hand_gripper_d")


@ddt
class TestSetHandGripperD(unittest.TestCase):
    ...
    def test_set_hand_gripper_d(self, case):
        ...
        # 请求发送
        set_res = self.device.ml.set_hand_gripper_d(case["joint"], case["parameter"])
        get_res = self.device.ml.get_hand_gripper_d(case["joint"])
        try:
            # 请求结果类型断言
            if type(set_res) == int:
    ...
    def test_out_limit(self, case):
        ...
        try:
            with self.assertRaises(MercuryDataException,
                ...
                self.device.ml.set_hand_gripper_d(case["joint"], case["parameter"])
