...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMyHand.TEST_DATA_FILE, "set_gripper_joint_torque")


@ddt
class TestSetGripperJointTorque(unittest.TestCase):
    # 实例化日志模块
    logger = logger
    ...
    def test_set_gripper_joint_torque(self, case):
        ...
        # 请求发送
        set_res = self.device.m.set_gripper_joint_torque(case["joint"],case["parameter"])
        get_res = self.device.m.get_gripper_joint_torque(case["joint"])
        # 请求结果类型断言
        if type(set_res) == int:
    ...
    def test_out_limit(self,case):
        ...
        # 请求发送
        try:
            with self.assertRaises(ValueError, msg="用例{}未触发value错误，扭矩值为{}".format(case['title'], case['parameter'])):
                self.device.m.set_gripper_joint_torque(case["joint"],case["parameter"])
