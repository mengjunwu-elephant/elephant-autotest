...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.TEST_DATA_FILE, "set_control_mode")


@ddt
class TestSetControlMode(unittest.TestCase):
    # 实例化日志模块
    logger = logger
    ...
    def test_set_control_mode(self, case):
        ...
        # 左臂请求发送
        l_response = self.device.ml.set_control_mode(case['parameter'])

        # 右臂请求发送
        r_response = self.device.mr.set_control_mode(case['parameter'])
        try:
            # 请求结果类型断言
    ...
    def test_out_limit(self, case):
        ...
        try:
            with self.assertRaises(MercuryDataException,
                                   msg="用例{}未触发value错误，运动模式为{}".format(case['title'], case['parameter'])):
                self.device.ml.set_control_mode(case['parameter'])
                self.device.mr.set_control_mode(case['parameter'])
    ...
    def test_save_or_not(self, case):
        ...
        # 读取默认值
        l_get_res = self.device.ml.get_control_mode()
        r_get_res = self.device.mr.get_control_mode()
        try:
            # 请求结果类型断言
            if type(l_response) == int:
                self.logger.debug('左臂请求类型断言成功')
