...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.PRO_GRIPPER_TEST_DATA_FILE, "set_pro_gripper_open")


@ddt
class TestGetProGripperOpen(unittest.TestCase):
    ...
    def tearDownClass(cls):
        cls.device.ml.set_pro_gripper_close()
        cls.device.close()
        logger.info("环境清理完成，接口测试结束")

    @data(*cases)
    def test_set_pro_gripper_open(self, case):
        ...
        # 请求发送
        response = self.device.ml.set_pro_gripper_open()
        try:
            # 请求结果类型断言
