...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestProGripper.TEST_DATA_FILE, "set_vir_pos")


@ddt
class TestSetVirPos(unittest.TestCase):
    # 实例化日志模块
    logger = logger
    ...
    def tearDownClass(cls):
        cls.device.m.set_gripper_vir_pos(8)
        cls.device.m.close()
    ...
    def test_set_vir_pos(self, case):
        ...
        # 请求发送
        set_res = self.device.m.set_gripper_vir_pos(case["parameter"])
        get_res = self.device.m.get_gripper_vir_pos()
        # 请求结果类型断言
        if type(set_res) == int:
