...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestProGripper.TEST_DATA_FILE, "get_firmware_version")
...
class TestGetFirmwareVersion(unittest.TestCase):
    ...
    @data(*cases)
    def test_get_firmware_version(self, case):
        ...
        # 请求发送
        response = self.device.m.get_firmware_version()
        try:
            # 请求结果类型断言
