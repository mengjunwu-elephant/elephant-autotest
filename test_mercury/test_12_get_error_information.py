...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.TEST_DATA_FILE, "get_error_information")
...
class TestGetErrorInformation(unittest.TestCase):
    ...
    def test_singular_point_error(self, case):
        ...
        # 左臂请求发送
        l_response = self.device.ml.get_error_information()

        # 右臂请求发送
        r_response = self.device.mr.get_error_information()
        try:
            # 请求结果类型断言
