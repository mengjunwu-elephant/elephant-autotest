...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestProGripper.TEST_DATA_FILE, "get_vir_pos")


@ddt
class TestGetQueueCount(unittest.TestCase):
    # 实例化日志模块
    logger = logger
    ...
    @data(*cases)
    def test_get_vir_pos(self, case):
        ...
        # 请求发送
        response = self.device.m.get_gripper_vir_pos()
        try:
            # 请求结果类型断言
