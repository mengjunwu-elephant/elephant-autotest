...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestProGripper.TEST_DATA_FILE, "set_gripper_baud")
...
class TestSetGripperBaud(unittest.TestCase):
    ...
    def tearDownClass(cls):
        cls.device = TestProGripper(baudrate=1000000)
    ...
    def test_set_gripper_baud(self, case):
        ...
        # 设置波特率
        set_res = self.device.m.set_gripper_baud(case["parameter"])
        # 重新连接夹爪
        self.device.m.close()
        self.device = TestProGripper(baudrate=baud[case["parameter"]])
        # 验证设置波特率是否设置成功
    ...
    def test_out_limit(self,case):
        ...
        # 请求发送
        try:
            with self.assertRaises(ValueError, msg="用例{}未触发value错误，baud值为{}".format(case['title'], case['parameter'])):
