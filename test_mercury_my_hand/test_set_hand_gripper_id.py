...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.MY_HAND_TEST_DATA_FILE, "set_hand_gripper_id")


@ddt
class TestSetHandGripperID(unittest.TestCase):
    ...
    def tearDownClass(cls):
        cls.device.ml.set_hand_gripper_id(14,gripper_id=10)
        cls.device.close()
        logger.info("环境清理完成，接口测试结束")

    @data(*[case for case in cases if case.get("test_type") == "normal"])  # 筛选有效等价类用例
    def test_set_hand_gripper_id(self, case):
        ...
        # 请求发送
        set_res = self.device.ml.set_hand_gripper_id(case["parameter"])
        get_res = self.device.ml.get_hand_gripper_id()
        try:
            # 请求结果类型断言
            if type(set_res) == int:
    ...
    def test_out_limit(self, case):
        ...
        try:
            with self.assertRaises(MercuryDataException,
                                   msg="用例{}未触发value错误，ID值为{}".format(case['title'], case['parameter'])):
                self.device.ml.set_hand_gripper_id(case['parameter'])
