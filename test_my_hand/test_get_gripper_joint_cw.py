...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMyHand.TEST_DATA_FILE, "get_gripper_joint_cw")


@ddt
class TestGetGripperJointCw(unittest.TestCase):
    # 实例化日志模块
    logger = logger
    ...
    def test_get_gripper_joint_cw(self, case):
        ...
        # 请求发送
        response = self.device.m.get_gripper_joint_cw(case["joint"])
    ...
    def test_out_limit(self,case):
        ...
        # 请求发送
        try:
            with self.assertRaises(ValueError, msg="用例{}未触发value错误，Cw值为{}".format(case['title'], case['joint'])):
                self.device.m.get_gripper_joint_cw(int(case['joint']))
