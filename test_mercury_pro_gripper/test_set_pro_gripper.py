...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.PRO_GRIPPER_TEST_DATA_FILE, "set_pro_gripper")


@ddt
class TestSetProGripper(unittest.TestCase):
    ...
    def tearDownClass(cls):
        # cls.device.ml.set_pro_gripper(3)
        cls.device.ml.close()
        logger.info("环境清理完成，接口测试结束")

    @data(*[case for case in cases if case.get("test_type") == "normal"])  # 筛选有效效等价类用例
    def test_set_pro_gripper(self, case):
        ...
        # 请求发送
        set_res = self.device.ml.set_pro_gripper(case["parameter"],case['value'])
        get_res = self.device.ml.get_pro_gripper(case["parameter"])
        try:
            # 请求结果类型断言
            if type(set_res) == int:
    ...
    def test_out_limit(self, case):
        ...
        try:
            with self.assertRaises(MercuryDataException,
                ...
                self.device.ml.set_pro_gripper(case["parameter"],case['value'])
