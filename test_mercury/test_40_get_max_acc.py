...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.TEST_DATA_FILE, "get_max_acc")


@ddt
class TestGetMaxAcc(unittest.TestCase):
    ...
    def test_get_max_acc(self, case):
        ...
        logger.debug('test_parameter:{}'.format(case['mode']))

        # 左臂请求发送
        l_response = self.device.ml.get_max_acc(case["mode"])

        # 右臂请求发送
        r_response = self.device.mr.get_max_acc(case["mode"])
        try:
            # 请求结果类型断言
    ...
    def test_out_limit(self, case):
        ...
        try:
            with self.assertRaises(MercuryDataException,
                                   msg="用例{}未触发value错误，参数为{}".format(case['title'], case['mode'])):
                self.device.ml.get_max_acc(case['mode'])
                self.device.mr.get_max_acc(case['mode'])
