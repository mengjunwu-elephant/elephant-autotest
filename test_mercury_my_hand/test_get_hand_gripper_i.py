...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.MY_HAND_TEST_DATA_FILE, "get_hand_gripper_i")


@ddt
class TestGetHandGripperJointI(unittest.TestCase):
    ...
    def test_out_limit(self, case):
        ...
        try:
            with self.assertRaises(MercuryDataException,
                                   msg="用例{}未触发value错误，I值为{}".format(case['title'], case['joint'])):
                self.device.ml.get_hand_gripper_i(int(case['joint']))
