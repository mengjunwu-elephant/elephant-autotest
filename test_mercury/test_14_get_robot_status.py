...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.TEST_DATA_FILE, "get_robot_status")


@ddt
class TestGetRobotStatus(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        """
        水星系列初始化先左臂上电，后右臂上电
    ...
    def test_get_robot_status1(self, case):
        ...
        # 左臂请求发送
        l_response = self.device.ml.get_robot_status()

        # 右臂请求发送
        r_response = self.device.mr.get_robot_status()
        try:
            # 请求结果类型断言
    ...
    def test_get_robot_status2(self, case):
        logger.info('》》》》》用例【{}】开始测试《《《《《'.format(case['title']))
        # 调试信息
        logger.debug('test_api:{}'.format(case['api']))
        logger.debug('test_parameter:{}'.format(case['parameter']))
