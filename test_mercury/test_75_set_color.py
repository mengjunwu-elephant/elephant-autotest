...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.TEST_DATA_FILE, "set_color")
...
class TestSetColor(unittest.TestCase):
    ...
    def test_set_color(self, case):
        ...
        # 左臂请求发送
        l_response = self.device.ml.set_color(eval(case['parameter']))

        # 右臂请求发送
        r_response = self.device.mr.set_color(eval(case['parameter']))
    ...
    def test_out_limit(self, case):
        ...
        try:
            with self.assertRaises(ValueError, msg="用例{}未触发value错误，id值为{}".format(case['title'], case['id'])):
                self.device.ml.set_color(eval(case["parameter"]))
                self.device.mr.set_color(eval(case["parameter"]))
