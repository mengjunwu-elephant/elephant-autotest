...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.TEST_DATA_FILE, "set_end_type")


@ddt
class TestSetEndType(unittest.TestCase):
    ...
    def tearDownClass(cls):
        ...
        :return:
        """
        cls.device.ml.set_end_type(0)
        cls.device.mr.set_end_type(0)
    ...
    def test_set_end_type(self, case):
        ...
        # 左臂请求发送
        l_response = self.device.ml.set_end_type(case['parameter'])

        # 右臂请求发送
        r_response = self.device.mr.set_end_type(case['parameter'])
        try:
            # 请求结果类型断言
    ...
    def test_out_limit(self, case):
        ...
        try:
            with self.assertRaises(MercuryDataException,
                ...
                self.device.ml.set_end_type(case['parameter'])
                self.device.mr.set_end_type(case['parameter'])
    ...
    def test_save_or_not(self, case):
        ...
        # 读取默认值
        l_get_res = self.device.ml.get_end_type()
        r_get_res = self.device.mr.get_end_type()
        try:
            # 请求结果类型断言
            if type(l_response) == int:
                logger.debug('左臂请求类型断言成功')
