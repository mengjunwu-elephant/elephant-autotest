...
# 从Excel中提取数据
cases = get_test_data_from_excel(TestMercury.TEST_DATA_FILE, "jog_increment_angle")


@ddt
class TestJogIncrementAngle(unittest.TestCase):
    ...
    def test_jog_increment_angle_left(self, case):
        ...
        # 左臂请求发送
        l_response = self.device.ml.jog_increment_angle(case['joint'], case["parameter"], case["speed"])
        try:
            # 请求结果类型断言
    ...
    @data(*[case for case in cases if case.get("test_type") == "normal"])
    def test_jog_increment_angle_right(self, case):
        logger.info('》》》》》用例【{}】开始测试《《《《《'.format(case['title']))
        # 调试信息
        logger.debug('test_api:{}'.format(case['api']))
        logger.debug('test_joint:{}'.format(case['joint']))
        ...
        logger.debug('test_parameter_2:{}'.format(case['speed']))
        # 右臂请求发送
        r_response = self.device.mr.jog_increment_angle(case['joint'], case["parameter"], case["speed"])
        try:
            # 请求结果类型断言
        ...
        try:
            with self.assertRaises(MercuryDataException,
                                   msg="用例{}未触发value错误，角度，速度为{}{}".format(case['title'], case['parameter'],
                                                                                      case['speed'])):
                # 左臂请求发送
                self.device.ml.jog_increment_angle(case['joint'], case["parameter"], case["speed"])
                # 右臂请求发送
                self.device.mr.jog_increment_angle(case['joint'], case["parameter"], case["speed"])
